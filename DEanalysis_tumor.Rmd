...
```{r}
table(coadse.tumor.filt$type)
```
We start from the filtered samples where there are only tumor samples.
```{r}
table(coadse.tumor.filt$gender)
```
Within these tumor samples we have 101 females and 121 males.
```{r}
table(coadse.tumor.filt$ajcc_pathologic_tumor_stage)
```
We want to focus on the differential expressed genes between tumor stages. 
...
In the follwing step we create the design matrix. Now we want to analyse wether there are differential expressed genes between males and females. The first step is to create the design matrix `mod0`. 
```{r}
...
fit$genes <- genesmd
head(tt)
#sort(table(tt$chr[tt$P.Value < FDRcutoff]), decreasing = TRUE) # adj.P.val is higher than p value
```

```{r}
sort(table(tt$chr[tt$P.Val < FDRcutoff]), decreasing = TRUE)
```
...
In the follwing step we create the design matrix. Now we want to analyse wether there are differential expressed genes between males and females. The first step is to create the design matrix `mod`. 

```{r}
mod2 <- model.matrix(~ajcc_pathologic_tumor_stage, colData(coadse.tumor.filt))
mod0 <- model.matrix(~1, colData(coadse.tumor.filt))
