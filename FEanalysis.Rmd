...
```{r scatter_plot, fig.align = 'center',  fig.height=12, fig.width=10, out.width="900px"}
...
genesGS10 <- rowData(coadse.filt)[top6G,1]
genesGS10
...
plotGS(coadse.filt, top1G, "type", main = paste("1)",names(rnkGS_Z)[1]),dge.filt,  cex.main = 1, cex.lab = 2, las = 1)
plotGS(coadse.filt, top2G, "type", main = paste("2)",names(rnkGS_Z)[2]),dge.filt, cex.main = 1, cex.lab = 2, las = 1)
plotGS(coadse.filt, top10G, "type", main = paste("3)", names(rnkGS_Z)[10]), dge.filt, cex.main = 1, cex.lab = 2, las = 1)
plotGS(coadse.filt, top15G, "type", main = paste("4)",names(rnkGS_Z)[15]),dge.filt, cex.main = 1, cex.lab = 2, las = 1)
plotGS(coadse.filt, top37G, "type", main = paste("5)",names(rnkGS_Z)[37]),dge.filt, cex.main = 1, cex.lab = 2, las = 1)
```

We can observe that in the selected genesets the distribution of the genes in these gene sets is mainly downregulated in tumor cases respect to normal. In general, gene downregulation in cancer could possibly be the cause of dysregulation of important pathways, repression of apoptosis or tumor suppressors' repression.

Now we want to investigate the specific genes that are up- and down-regulated.

In the plot 1 of figure @ref(fig:scatter_plot), we can observe an overexpression of the MEF2A transcription factor myocyte-enhancer factor 2 known to play a role in adaptive responses during development and adult life. Even if the specific role in tumorgenesis of the MEF2 family has not been clarified yet, it has been identified that it can favor matrix degradative processes when its activation is promoted by TGF-?? by decreasing the stability of HDACs [Di Giorgio et al.](https://www.sciencedirect.com/science/article/pii/S0304419X18300702). Consistently with this scenario we observe an underexpression of the HDAC2 gene, which competes for binding to the same region of MEF2.
 
In the plot 2 of figure @ref(fig:scatter_plot) we observe an overexpression of the HT receptor (HTR4) which is involved in the neuronal response with the serotonergic synapse pathway [Arese et al.](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5890045/). HT receptors have been shown to be over expressed in cancer tissues and that their antagonists inhibit the HT effect to different extents and induce apoptosis [Radin et al.]( https://www.ncbi.nlm.nih.gov/pubmed/28088112).

In plot 3 of figure @ref(fig:scatter_plot) we can identify an overexpression of two Frizzled Receptors(FZD7 and FZD9). 
FZD9 expression was reported to be upregulated in different carcinomas [Qiu et al.](https://www.ncbi.nlm.nih.gov/pubmed/27852064/). Moreover, a dysregulation of the WNT pathway by FZD7 was related to tumorigenesis and metastasis. As mentioned, a possible disregulation of the WNT pathway is induced by the Frizzled Receptors; consistently to this we observe that expression levels of WNT10a and WNT6 are affected as well in tumor patients. A desregulation in Wnt pathway has been associated with the accumulation of the oncogenic protein beta-catein, and therefore, with the development of cancer [Bogaert et al.](https://www.ncbi.nlm.nih.gov/pubmed/24714764).   

In plot 4 of figure @ref(fig:scatter_plot) we identify an overexpression of the PDGF-B gene.In a mice study was identified that PDGF???B released from colon tumor cells regulated tumor growth by inducing blood vessel formation. Also they found that an elevated expression of PDGF???B was also correlated with tumor size [Hsu et al.](https://onlinelibrary.wiley.com/doi/abs/10.1002/jcp.1041650204).

In plot 5 of figure @ref(fig:scatter_plot) we identify an overexpression of the CENPA gene. Recent work has demonstrated that the kinetochore protein CENP-A was overexpressed in all of 11 primary human colorectal cancer tissues. It is also known that chromosome missegregation during mitosis is the main cause of aneuploidy and contributes to oncogenesis. Centromere protein (CENP)-A is the centromere-specific histone-H3-like variant essential for centromere structure, function and the assembly of the kinetochore [Tamonaga et al.](https://www.ncbi.nlm.nih.gov/pubmed/12839935).
