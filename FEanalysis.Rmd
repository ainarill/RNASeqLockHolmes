...
One characteristic of the rapidly growing tumor cells is the capacity to sustain high rates of glycolysis for ATP generation. Also, the alteration of some methabolic patways to facilitate the uptake and incorporation of abundant nutrients, such as glucose, nucleotides, aminoacids and lipids, are essential for highly proliferating cells [@satoh2017global].

From a first point of view, we can see that potentially molecular changes are done in pathways related with cell cycle (glucolysis, fatty acid metabolism and purine methabolism) and homeostasis maintaining. For instance, genes related with the **Wnt** signaling pathway seems to be affected, which has been related to tissue homeostasis and repair. A desregulation in Wnt pathway has been associated with the accumulation of the oncogenic protein *beta-catein*, and therefore, with the development of cancer[@bogaert2014molecular].   

Another feature that can be pointed out is the downregulation of proteins related with apoptosis. For instance, downregulation in CASP7 gene (caspase 7) in tumor samples respect to normal has been associated with colon adenocarcinoma developemnt [@alfarouk2014glycolysis]. The Caspase signaling are well known to been associated with cell apoptosis. Thus, we could expect this gene to be down regulated, leading to the repression of apoptosis in tumoral cells.

Now, we plot the mean expression values per gene for 6 of the above mentioned pathways, that we consider can be interesting to  analyse.
```{r}
head(rnkGS_Z)
```

```{r scatter_plot1, fig.align = 'center'}
...
top3G <- colnames(Im)[which(Im[names(rnkGS_Z)[3], ] == 1)]
genesGS3 <- rowData(coadse.filt)[top3G,1]
top4G <- colnames(Im)[which(Im[names(rnkGS_Z)[4], ] == 1)]
genesGS4 <- rowData(coadse.filt)[top4G,1]
top5G <- colnames(Im)[which(Im[names(rnkGS_Z)[5], ] == 1)]
genesGS5 <- rowData(coadse.filt)[top5G,1]
top6G <- colnames(Im)[which(Im[names(rnkGS_Z)[6], ] == 1)]
genesGS6 <- rowData(coadse.filt)[top6G,1]
par(mfrow = c(2, 3), mar= c(0.5,5,2,0.5), mai=c(0.5,0.6,0.6,0.5))
plotGS(coadse.filt, top1G, "type", dge.filt, main = names(rnkGS_Z)[1], cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, top2G, "type", dge.filt, main = names(rnkGS_Z)[2], cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, top3G, "type", dge.filt, main = names(rnkGS_Z)[3], cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, top4G, "type", dge.filt, main = names(rnkGS_Z)[4], cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, top5G, "type", dge.filt, main = names(rnkGS_Z)[5], cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, top6G, "type", dge.filt, main = names(rnkGS_Z)[6], cex.main = 0.7, cex.lab = 2, las = 1)
```

We can see in figure \@ref(fig:scatter_plot1) that, in general, the distribution of the genes in these gene sets are mainly down-regulated in tumor cases respect to normal.
...
par(mfrow = c(2, 3), mar= c(0.5,5,2,0.5), mai=c(0.5,0.6,0.6,0.5))
plotGS(coadse.filt, topgs1genes, "type", dge.filt,  main = names(rnkGS)[1],cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, topgs2genes, "type", dge.filt, main = names(rnkGS)[2],cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, topgs3genes, "type", dge.filt, main = names(rnkGS)[3],cex.main = 0.8, cex.lab = 2, las = 1)
plotGS(coadse.filt, topgs4genes, "type", dge.filt, main = names(rnkGS)[4],cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, topgs5genes, "type", dge.filt, main = names(rnkGS)[5],cex.main = 0.7, cex.lab = 2, las = 1)
plotGS(coadse.filt, topgs6genes, "type", dge.filt, main = names(rnkGS)[6],cex.main = 0.7, cex.lab = 2, las = 1)
