...
LUISA COMMENTS: if we dont filter for large size, we observe genesets involved in odor perception -- proven its true. 

 - some very important genes in term of DE --> We can look up what they are involved in 
...
We are computing the significance of a GO term conditional on the significance of its children.
```{r}
DEgenes <- rownames(tt)[tt$adj.P.Val < 0.1]
length(DEgenes)
geneUniverse <- rownames(lclse.filt)
# BIG AIM: perform functional enrichment analysis on the entire collection of GO gene sets 
...
We then explore some of the results.
```{r}
# exploring
head(summary(hgOver))
...
head(pvalues(hgOver))
```
In general, genest which are either very small or that present a small number of DE genes are not really reliable. On the other hand, if the geneset is too big, its enrichment might provide little insights. In order to correct for this we filter out the goresults.  
```{r}
goresults <- goresults[goresults$Size >= 3 & goresults$Size <= 300 & goresults$Count >= 3, ] 
goresults <- goresults[order(goresults$OddsRatio, decreasing=TRUE), ]
head(goresults)
```
Let's extract the genes that enrich each GO term 
```{r}
...
gsc <- mapIdentifiers(gsc, AnnoOrEntrezIdentifier(metadata(lclse.filt)$annotation))
