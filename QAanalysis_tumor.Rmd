...
ROBERT DICE QUE SE TIENE QUE HACER UN MDS PLOT, PERO LO HE ESTADO BUSCANDO Y NO SE QUÉ REPRESENTARIA CADA EJE (COMMAND: cmdscale)

```{r}
par(mfrow=c(1,2))
col.stage <- c("blue","cyan", "orange", "purple")[factor(coadse.tumor.filt$ajcc_pathologic_tumor_stage)]
head(data.frame(coadse.tumor.filt$ajcc_pathologic_tumor_stage, col.stage))
batch <- as.integer(factor(tss.tumor))
plotMDS(dge.tumor.filt, dim=c(3,4), col=col.stage, pch= batch,cex=2)
legend("topright", legend=levels(factor(coadse.tumor.filt$ajcc_pathologic_tumor_stage)), col=col.stage ,pch=16, cex= 0.75)
legend("bottomright", legend=levels(factor(tss.tumor)),pch=c(1,4), cex=0.75)
```
