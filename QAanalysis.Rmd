...
dim(colData(coadse))
```
We have a total of 524 samples that have 549 clinical variables/factors that could be analysed. However, we are not going to explore all these variables, but those that can have more relevance in the data of study.
...
substr(colnames(coadse.paired),9,12)
...
```{r}
genes.tumor <- data.frame(chr = as.character(seqnames(rowRanges(coadse.tumor))), symbol = rowData(coadse.tumor)[,
...
```
```{r libsizes, echo=FALSE, out.width="600px", fig.cap="Library sizes in increasing order."}
sampledepth2 <- round(dge.tumor$sample$lib.size / 1e6, digits=1)
names(sampledepth2) <- substr(colnames(coadse.tumor), 6, 12)
sort(sampledepth2)
par(mfrow = c(1, 1), mar = c(4, 5, 1, 1))
ord2 <- order(dge.tumor$sample$lib.size)

ggplot(as.data.frame(dge.tumor$samples), aes(x = row.names(dge.tumor$samples), y = dge.tumor$samples$lib.size[ord2]/1000000, fill = dge.tumor$samples$group[ord2])) + geom_bar(stat = "identity") + theme_bw() + theme(legend.title = element_blank(), axis.text.x=element_blank()) + ylab("Milions of reads") + xlab("Samples")
...
```{r}
CPM2 <- t(t(dge.tumor$counts)/(dge.tumor$samples$lib.size/1e+06))
...
```{r}
par(mfrow = c(1, 2), mar = c(4, 5, 1, 1))
multidensity(as.list(as.data.frame(assays(coadse.tumor)$counts/1e6)), xlab = "CPM", legend = NULL, main = "", cex.axis = 1.2, cex.lab = 1.5, las = 1)
multidensity(as.list(as.data.frame(assays(coadse.tumor)$logCPM)), xlab = "log2 CPM", legend = NULL, main = "", cex.axis = 1.2, cex.lab = 1.5, las = 1)

par(mfrow=c(2, 2))
multidensity(as.list(as.data.frame(assays(coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage I"])$logCPM)), xlab="log 2 CPM", legend=NULL, main="Stage I", las=1)
multidensity(as.list(as.data.frame(assays(coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage II"])$logCPM)), xlab="log 2 CPM", legend=NULL, main="Stage II", las=1)
multidensity(as.list(as.data.frame(assays(coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage III"])$logCPM)), xlab="log 2 CPM", legend=NULL, main="Stage III", las=1)
multidensity(as.list(as.data.frame(assays(coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage IV"])$logCPM)), xlab="log 2 CPM", legend=NULL, main="Stage IV", las=1)
```
...
AINA NOT SAME NUMBERS LOOK AGAIN AFTER.

```{r}
maskgenes.tumor <- avgexp > 1
dim(coadse.tumor)
## [1] 20115   222
coadse.tumor <- coadse.tumor[mask, ]
dim(coadse.tumor)
## [1] 11374   222
dim(dge.tumor2)
## [1] 20115   222
dge.tumor2 <- dge.tumor2[mask, ]
dim(dge.tumor2)
```
## Normalization
...
```{r}
dge.tumor.filt <- calcNormFactors(dge.tumor2) 
assays(coadse.tumor)$logCPM <- cpm(dge.tumor.filt, log=TRUE, normalized.lib.sizes=TRUE, prior.count=0.25)
```

```{r}
saveRDS(coadse.tumor, file.path("results", "coadse.tumor.rds"))
...
Stage1 <- coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage I"]
par(mfrow=c(15, 3), mar=c(4, 5, 3, 1))
...
For Stage II --> 
```{r maPlotsTumor, fig.height=36, fig.width=6, dpi=100, echo=FALSE, fig.cap="MA-plots of the tumor samples in Stage I."}
Stage2 <- coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage II"]
par(mfrow=c(15, 3), mar=c(4, 5, 3, 1))
...
For Stage III --> 
```{r maPlotsTumor, fig.height=36, fig.width=6, dpi=100, echo=FALSE, fig.cap="MA-plots of the tumor samples in Stage I."}
Stage3 <- coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage III"]
par(mfrow=c(15, 3), mar=c(4, 5, 3, 1))
...
For Stage IV --> 
```{r maPlotsTumor, fig.height=36, fig.width=6, dpi=100, echo=FALSE, fig.cap="MA-plots of the tumor samples in Stage I."}
Stage4 <- coadse.tumor[, coadse.tumor$ajcc_pathologic_tumor_stage == "Stage IV"]
par(mfrow=c(15, 3), mar=c(4, 5, 3, 1))
